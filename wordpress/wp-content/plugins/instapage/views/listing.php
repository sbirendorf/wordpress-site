<div class="c-form-text-item  c-form-text-item--no-label">
  <input type="text" class="c-form-text-item__field c-form-text-item__field--with-icon" placeholder="<?php echo Connector::lang( 'Search query' ); ?>" data-bind="value: query, valueUpdate: 'keyup'">
  <div class="c-form-text-item__bar"></div>
  <button class="c-button c-button--clean c-button--large c-form-text-item__button">
    <i class="material-icons c-button__icon"><?php echo Connector::lang( 'search' ); ?></i>
  </button>
  <span class="c-form-text-item__info">
    <span><?php echo Connector::lang( 'Search for page title or URL' ); ?></span>
  </span>
</div>
<h2 class="ui-subtitle"><?php echo Connector::lang( 'Page listing' ); ?></h2>
...
<script type="text/html" id="page-row-template">
  <tr>
    ...
    <td class="c-table__cell">
      ...
      <div class="page-url" data-bind="html: 'URL: <a href=\'<?php echo Connector::getHomeUrl();?>/' + $data.slug + '\' target=\'_blank\'/><?php echo Connector::getHomeUrl();?>/' + $data.slug + '</a>'"></div>
      <div class="page-actions">
        <div class="before-delete" data-bind="visible: !toDelete()">
          <button class="fx-ripple-effect c-button c-button--flat c-button--small" data-bind="click:masterModel.toolbarModel.loadEditPage"><?php echo Connector::lang( 'Edit' ); ?></button>
          <button class="fx-ripple-effect c-button c-button--flat c-button--danger c-button--small" data-bind="click:masterModel.pagedGridModel.askForDeleteConfirmation"><?php echo Connector::lang( 'Delete' ); ?></button>
        </div>
        <div class="after-delete" data-bind="visible: toDelete()">
          <button class="fx-ripple-effect c-button c-button--flat c-button--small" data-bind="click:masterModel.pagedGridModel.cancelDelete"><?php echo Connector::lang( 'Cancel' ); ?></button>
          <button class="fx-ripple-effect c-button c-button--flat c-button--danger c-button--small" data-bind="click:masterModel.pagedGridModel.deletePage"><?php echo Connector::lang( 'Confirm delete' ); ?></button>
        </div>
      </div>
    </td>
    ...
    <td data-bind="visible: statsLoaded() && typeof stats_cache() === 'undefined'" class="c-table__cell">
      <?php echo Connector::lang( 'Stats are unavailable at the moment.' ); ?>
    </td>
